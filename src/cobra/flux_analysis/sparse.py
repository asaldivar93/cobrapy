...
def find_leaks(model, keep_boundaries=False):
    ...
    for var in sparse_model.variables:
        if 'aux' in var.name:
            if abs(var.primal) > zero_cutoff:
                leaks.append([var.name.replace('aux_', ''), var.primal])

    return leaks


def find_leak_mode(model, leaks=[], cutoff_mult=1, keep_boundaries=False):
    ...
    for leak, flux in leaks:
        ...
        met_var = sparse_model.variables.get("aux_{}".format(leak))
        met_var.lb = flux
        sol = sparse_model.optimize()
        met_var.lb = 0

        rxns_in_mode = [[rxn.id, sol.fluxes[rxn.id]] for rxn in sparse_model.reactions
